...
def noise_to_x0(noise, xt, batched_t, ddpm):
    assert (
        batched_t.shape[0] == xt.shape[0] == noise.shape[0]
    ...
    x0 = 1.0 / sqrt_alpha_bar * xt - jnp.sqrt(1.0 / alpha_bar - 1) * noise
    return x0
...
def ddpm_sample_step(state, rng, x, t, condition, ddpm_params):
    ...
    x_mean = (
        1
        / jnp.sqrt(ddpm_params["alphas"][t])
        * (x - ddpm_params["betas"][t] / ddpm_params["sqrt_1m_alphas_bar"][t] * v)
    )
    var = (
        (1 - ddpm_params["alphas_bar"][t - 1])
        / (1 - ddpm_params["alphas_bar"][t])
        * ddpm_params["betas"][t]
    )
