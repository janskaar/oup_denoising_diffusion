...
def model_predict(state, x, t, ddpm_params):
    variables = {'params': state.params}
    noise_pred = state.apply_fn(variables, x, t)
    return noise_pred
...
def ddpm_sample_step(state, rng, x, t, ddpm_params):
    ...
    v = model_predict(state, x, batched_t, ddpm_params)
...
def sample_loop(rng, state, shape, sample_step, timesteps, z=None, x0=None):
    ...
    for t in reversed(jnp.arange(timesteps)):
        ...
        x, x_mean = sample_step(state, step_rng, x, t)
