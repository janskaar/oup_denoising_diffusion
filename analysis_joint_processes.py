...
cov = compute_ou_covariance(delta_s, params)

sample = stats.multivariate_normal.rvs(mean=np.zeros(2048), cov=cov, size=10)

cov_100 = compute_ou_cov_diffusion_t(ddpm_params["alphas_bar"][100], delta_s, params)
...
cov_sim = compute_temporal_covariances(zs[:,0,:], zs)
cov_sim_100 = compute_temporal_covariances(zs_100[:,0,:], zs_100)
